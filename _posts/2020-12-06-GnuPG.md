...
In the above example, I push my public key to keys.gnupg.net, at which point I then also attempted to pull they key from the server. When pulling they key from the server you may need to include `0x` in front of the strong for it to work. You can then also go to keys.gnupg.net and search for your key ID to verify it was uploaded.
